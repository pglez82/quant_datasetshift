...
import quapy as qp
import numpy as np
from utils.covariate_shift_protocol_cluster import CovariateShiftPPCluster
...
with qp.util.temp_seed(seed):
    ...
    for n_p_train, p_train in enumerate(ps_train):
        ...
        trainSampleGenerator = CovariateShiftPPCluster(domainA_train, domainB_train, prevalence=(1-p_train,p_train), sample_size = training_sample_size, return_type="labelled_collection",mixture_points=mixture_points,repeats=n_reps_train,random_state=seed)
        ...
        for n_training_sample, training_sample in enumerate(trainSampleGenerator()):
            ...
            for p_test in ps_test:
                for quant_name, quantifier in quant_methods.items():
                    ...
                    testSampleGenerator = CovariateShiftPPCluster(testA, testB, sample_size = test_sample_size, mixture_points=mixture_points, prevalence=(1-p_test,p_test), repeats=n_test_samples, random_state=seed)
                    for n_test_sample, test_sample in enumerate(testSampleGenerator()):
                        ...
                        true = test_sample[1]
                        if len(true)>2:
                            true = np.array([true[0]+true[2],true[1]+true[3]]) 
    ...
    for quant_name, quantifier in quant_methods.items():
        ...
        experiment_results[quant_name].to_csv("results/covariate_cluster/results_%s_%s.csv" % (date_string,quant_name))
