...
with qp.util.temp_seed(seed):
    ...
    for method_name in quant_methods.keys():
        experiment_results[method_name] = pd.DataFrame(columns=["p_train","p_test","train_rep","test_sample","p_hat","error"])
    ...
    for n_training_sample, training_sample in enumerate(trainSampleGenerator()):
        ...
        for quant_name, quantifier in quant_methods.items():
            grids[quant_name] = qp.model_selection.GridSearchQ(quantifier,param_grid=param_grid,protocol=APP(valsplit,sample_size=test_sample_size,n_prevalences=n_prevalences, random_state=seed),refit=True,verbose=False).fit(trainsplit)
        print("Done. Evaluating...")   
        for quant_name, quantifier in quant_methods.items():
            print("Evaluating quantifier %s" % quant_name)
            testSampleGenerator = APP(testtranformed, sample_size = test_sample_size,n_prevalences=n_prevalences, repeats=n_test_samples, random_state=seed)
